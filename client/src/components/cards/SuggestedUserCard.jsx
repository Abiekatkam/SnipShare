...
const SuggestedUserCard = ({ data, handleUnfollow, type }) => {
  console.log(data);
  return (
    <div className="w-full flex items-center justify-between gap-3 rounded-xl p-2 px-3 font-semibold text-lg tracking-wide dark:text-white transition-all bg-slate-200 text-[#09090b] ease-in dark:bg-[#27272a]/50 mb-2">
      ...
      >
        ...
        <div className="w-fit h-full flex flex-col items-start">
          ...
          {data?.bio && <span className="text-xs text-pretty text-slate-400">{data?.bio}</span>}
        </div>
      </Link>
      <Button className="h-7 text-sm bg-[#09090b] dark:bg-white capitalize" onClick={()=>handleUnfollow(data?._id)}>
        {type}
      </Button>
    </div>
  );
